...
std::pair<bitvec_t, wvec_t> postselect_bitstrings_cpp(
    const bitvec_t &bitstrings, const wvec_t &weights, const uint32_t &right, const uint32_t &left)
{
...
bitvec_t subsample_cpp(
    const bitvec_t &bitstrings, const wvec_t &weights, const unsigned int &samples_per_batch, const uint32_t &seed)
{
    std::mt19937 rng(seed);
    return Qiskit::addon::sqd::subsample(
        bitstrings, weights, samples_per_batch, rng);
}

std::pair<bitvec_t, wvec_t> recover_configurations_cpp(
    const bitvec_t &bitstrings,
