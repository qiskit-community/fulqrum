...
inline void sort_groups_by_ladder_int(QubitOperator_t& oper,
                                      ...
                                      unsigned int ladder_width)
    {
        ...
        for(kk=0; kk < num_groups; kk++)
        {
            ...
            std::sort(&oper.terms[start], &oper.terms[stop], [=](const OperatorTerm_t& a, const OperatorTerm_t& b)
                                  {
...
void ladder_bin_starts(const std::vector<OperatorTerm_t>& terms, const std::size_t * group_ptrs,
                        unsigned int * group_counts, std::size_t * group_ranges,
                        unsigned int num_groups, unsigned int num_ladder_bins, unsigned int ladder_width)
{
