...
As shown below, a typical eigenvalue problem is broken up into three pieces: 1) Bit-strings are collected from a quantum computer (classical methods also work), and optionally post-processed.  2) Fulqrum converts these bit-strings into a ``Subspace`` that together with the Hamiltonian (``QubitOperator``) that describes the system are used to generate a ``SubspaceHamiltonian`` from which numerical representations can be generated. Fermionic systems are first transformed to qubit Hamiltonians via an efficient Jordan-Wigner mapping.  Finally, step (3) takes these represenations and uses them in standard eigensolving packages.  In this way, Fulqrum allows for flexibility in generating solutions that other methods lack. 
